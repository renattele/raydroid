...
internal fun Note.keywords(): List<String> = (title.splitWords() + body.splitWords()).distinct().take(16)
...
private fun String.splitWords(): List<String> = split(Regex("\\s+")).map { word -> word.trim() }.filter { word -> word.length > 2 }
