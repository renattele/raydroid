...
class SearchAliasRepositoryImplTest {
    @Test
    fun `save normalizes alias and resolve trims lookup`() =
        runTest {
    ...
    @Test
    fun `save rejects duplicates owned by another target`() =
        runTest {
            val repository = repository(testScheduler)
    ...
    @Test
    fun `concurrent duplicate saves keep a single winner`() =
        runTest {
            val repository = repository(testScheduler)
            val first = resultId("ru.raydroid.calculator", "calculator", CommandItemId.CommandRoot)
            val second = resultId("ru.raydroid.notes", "notes", CommandItemId.CommandRoot)
    ...
    @Test
    fun `save rejects aliases with whitespace`() =
        runTest {
            val repository = repository(testScheduler)
            val result =
    ...
    @Test
    fun `save rejects non ascii aliases`() =
        runTest {
            val repository = repository(testScheduler)
            val result =
                repository.saveAlias(
    ...
    @Test
    fun `remove clears stored alias`() =
        runTest {
            val repository = repository(testScheduler)
            val resultId = resultId("ru.raydroid.calculator", "calculator", CommandItemId.CommandRoot)
    ...
    @Test
    fun `observe canonicalizes duplicate aliases already stored`() =
        runTest {
    ...
    @Test
    fun `observe removes non ascii aliases that visually match ascii aliases`() =
        runTest {
            val scope = CoroutineScope(StandardTestDispatcher(testScheduler) + SupervisorJob())
            val store =
                PreferenceDataStoreFactory.createWithPath(scope = scope) {
