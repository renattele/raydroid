...
import ru.raydroid.plugin.api.manifest.SystemAccessPermission
...
internal class PermissionSystemServiceBridge(
    ...
    override suspend fun getApps(): List<SystemServiceBridge.RawApplication> {
        if (manifest.access.system
                ?.permissions
                ?.contains(SystemAccessPermission.GetApps) != true
        ) {
        ...
        val visibleApps =
            manifest.access.system?.visibleApps?.let {
                visibleApps(
                    it,
    ...
    override suspend fun openApp(
        ...
        if (manifest.access.system
                ?.permissions
                ?.contains(SystemAccessPermission.OpenApp) != true
        ) {
            throw PermissionDenied()
        }
    ...
    override suspend fun open(
        ...
        if (manifest.access.system
                ?.permissions
                ?.contains(SystemAccessPermission.OpenResource) != true
        ) {
            throw PermissionDenied()
        }
        if (manifest.access.system
                ?.allowedUrls
                ?.let { hasAccessToUrl(it, target) } == true
        ) {
