...
internal fun UiText.toPluginUiText(pluginId: PluginId): PluginUiText =
    when (type) {
        UiText.Type.Plain -> PluginUiText.Plain(text)
        UiText.Type.Resource -> PluginUiText.Resource(pluginId = pluginId, key = text)
    }

internal fun Icon.toPluginIcon(pluginId: PluginId): PluginIcon =
    when (type) {
...
internal fun Image.toPluginImage(pluginId: PluginId): PluginImage =
    when (this) {
        is Image.Url -> PluginImage.Url(url)
        is Image.Resource -> PluginImage.Resource(pluginId = pluginId, key = resource)
    }

internal fun Color.toPluginColor(): PluginColor =
    when (this) {
...
internal fun FontSize.toPluginFontSize(): PluginFontSize =
    when (this) {
...
internal fun FontWeight.toPluginFontWeight(): PluginFontWeight =
    when (this) {
        FontWeight.Normal -> PluginFontWeight.Normal
        FontWeight.Bold -> PluginFontWeight.Bold
    }

internal fun IconSize.toPluginIconSize(): PluginIconSize =
    when (this) {
...
internal fun ShapeToken.toPluginShapeToken(): PluginShapeToken =
    when (this) {
...
internal fun MotionToken.toPluginMotionToken(): PluginMotionToken =
    when (this) {
...
internal fun Spacing.toPluginSpacing(): PluginSpacing =
    when (this) {
...
internal fun Alignment.toPluginAlignment(): PluginAlignment =
    when (this) {
...
internal fun BoxAlignment.toPluginBoxAlignment(): PluginBoxAlignment =
    when (this) {
...
internal fun Orientation.toPluginOrientation(): PluginOrientation =
    when (this) {
        Orientation.Vertical -> PluginOrientation.Vertical
        Orientation.Horizontal -> PluginOrientation.Horizontal
    }

internal fun Arrangement.toPluginArrangement(): PluginArrangement =
    when (this) {
...
internal fun RayNodeData.toPluginRayNodeData(
    ...
    when (this) {
        ...
        is DetailData -> {
            toPluginDetailData(pluginId).withModifier(modifier.toPluginRayModifier(pluginId, dispatchCallback))
        }

        is EditableTextData -> {
            PluginEditableTextData(
                id = id,
                value = value,
                selection = selection,
                ...
                onChange =
                    PluginFormSubmitCallback { values ->
...
private fun DetailData.toPluginDetailData(pluginId: PluginId): PluginDetailData =
    PluginDetailData(
        markdown = markdown,
...
private fun DetailMetadataItemData.toPluginDetailMetadataItemData(pluginId: PluginId): PluginDetailMetadataItemData =
    when (this) {
        is DetailMetadataItemData.Label -> {
            PluginDetailMetadataItemData.Label(
                title = title.toPluginUiText(pluginId),
                text = text?.toPluginUiText(pluginId),
                icon = icon?.toPluginIcon(pluginId),
            )
        ...
        is DetailMetadataItemData.Link -> {
            PluginDetailMetadataItemData.Link(
                title = title.toPluginUiText(pluginId),
                text = text.toPluginUiText(pluginId),
                target = target,
            )
        ...
        is DetailMetadataItemData.TagList -> {
            PluginDetailMetadataItemData.TagList(
                title = title.toPluginUiText(pluginId),
                tags =
                    tags.map { tag ->
        ...
        DetailMetadataItemData.Separator -> {
            PluginDetailMetadataItemData.Separator
        }
    }

private fun FormFieldData.toPluginFormFieldData(pluginId: PluginId): PluginFormFieldData =
    when (this) {
        is FormFieldData.TextField -> {
            PluginFormFieldData.TextField(
                id = id,
                title = title?.toPluginUiText(pluginId),
                required = required,
                placeholder = placeholder?.toPluginUiText(pluginId),
        ...
        is FormFieldData.Dropdown -> {
            PluginFormFieldData.Dropdown(
                id = id,
                title = title?.toPluginUiText(pluginId),
                required = required,
                options =
        ...
        is FormFieldData.DatePicker -> {
            PluginFormFieldData.DatePicker(
                id = id,
                title = title?.toPluginUiText(pluginId),
                required = required,
                defaultValue = defaultValue,
        ...
        is FormFieldData.Separator -> {
            PluginFormFieldData.Separator(id)
        }

        is FormFieldData.Description -> {
            PluginFormFieldData.Description(id, text.toPluginUiText(pluginId))
        }
    }
...
private fun GridAspectRatio.toPluginGridAspectRatio(): PluginGridAspectRatio =
    when (this) {
...
private fun FormSubmitStyle.toPluginFormSubmitStyle(): PluginFormSubmitStyle =
    when (this) {
...
private fun ActionPanelHintMode.toPluginActionPanelHintMode(): PluginActionPanelHintMode =
    when (this) {
...
private fun PluginFormValues.toApiFormValues(): FormValues =
    mapValues { (_, value) ->
        when (value) {
...
private fun CommandListAction.Style.toPluginStyle(): PluginCommandListAction.Style =
    when (this) {
...
private fun CommandCallbackRef.toPluginCommandCallback(dispatchCallback: suspend (CommandCallbackRef) -> Unit): PluginCommandCallback =
