...
internal class PermissionEnvironmentServiceBridge(
    ...
    override fun get(key: String): String? {
        if (manifest.access.environment?.permissions?.readable() == true) {
            if (manifest.access.environment?.access?.let { hasAccess(it, key) } == true) {
    ...
    private fun hasAccess(accessList: List<String>, key: String): Boolean {
        return accessList.any { accessString ->
