...
class SearchUseCase(
    ...
    operator fun invoke(
        ...
        return channelFlow {
            ...
            combine(commandsFlow, contentFlow, cachedResultsFlow, aliasesFlow) { commandItems, liveResults, cachedResults, aliases ->
                val rankedCommandResults =
                    searchResultRanker.rankCommands(
                        query = query,
                        commandsSnapshot = commandItems,
                        limit = limit,
                    )
                val rankedLiveResults =
                    searchResultRanker.rankLive(
                        query = query,
    ...
    private suspend fun promoteExactAlias(
        ...
        val runtimeCoordinator = pluginRuntimeRegistry.get()
        val aliasResultId =
            SearchAliasNormalizer
                .normalizeLookup(query)
                ?.let { alias ->
                    snapshot.aliases.entries
                        .firstOrNull { entry -> entry.value == alias }
                        ?.key
                }
                ?: return snapshot.results
        val promoted =
            snapshot.results.firstOrNull { result -> result.resultId == aliasResultId }
    ...
    private suspend fun refreshCachedAliasResult(
        ...
        val runtime =
            runtimeCoordinator.runtimes().value.firstOrNull { candidate ->
                candidate.pluginId == resultId.pluginId
            } ?: return null
        val refreshedMutations =
            runtime.cachedItems(
...
private fun List<SearchResultSet.SearchResult>.decorateAliases(aliases: Map<SearchResultId, String>): List<SearchResultSet.SearchResult> =
    map { result ->
