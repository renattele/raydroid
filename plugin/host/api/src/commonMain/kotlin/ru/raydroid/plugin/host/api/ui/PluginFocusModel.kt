...
fun List<PluginRayNodeData>.suppressesHostActions(): Boolean = any { node -> node.suppressesHostActions() }
...
private fun PluginRayNodeData.pluginFocusableItems(query: String): List<PluginFocusableItem> =
    when (this) {
        is PluginListData -> {
            filtered(query).flatMap { section ->
        ...
        is PluginBoxData -> {
            children.pluginFocusableItems(query)
        }
