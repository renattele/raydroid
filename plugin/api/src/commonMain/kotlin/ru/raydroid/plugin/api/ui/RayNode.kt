...
fun Modifier.enabled(enabled: Boolean): Modifier = then(EnabledElement(enabled))

fun Modifier.onClick(onClick: suspend () -> Unit): Modifier = then(ClickElement(onClick))

fun Modifier.actions(actions: CommandActionScope.() -> Unit): Modifier = then(ActionsElement(actions))

fun Modifier.weight(weight: Float): Modifier = then(WeightElement(weight))

fun Modifier.fillMaxSize(): Modifier = then(FillMaxSizeElement)

fun Modifier.padding(spacing: Spacing): Modifier = then(PaddingElement(spacing))
...
private fun buildRayNodes(
    ...
    val scope =
        object : RayScope {
            ...
            override fun registerFormCallback(
                path: String,
                callback: suspend (FormValues) -> Unit,
            ): CommandCallbackRef = registerFormCallback("$nodePath:form:$path", callback)
        }
    scope.content()
    return nodes
}
...
internal fun Modifier.elements(): List<Modifier.Element> =
    when (this) {
        Modifier -> emptyList()
...
internal fun Modifier.hasActions(): Boolean = elements().any { element -> element is ActionsElement }
...
internal fun Modifier.toRayModifier(
    ...
    return RayModifier(
        ...
        click =
            elements.filterIsInstance<ClickElement>().lastOrNull()?.let { element ->
                registerCallback("$path:click", element.onClick)
            },
        actions =
            actions.flatMapIndexed { index, element ->
                buildCommandActions(
